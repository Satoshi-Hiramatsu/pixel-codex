...
| 見た目 | 撮影対象の一覧、スマホ幅／PC幅の切り替え、撮れた画面の表示 |
...
- PCの画面を要求して表示（同上）。`preview.request`で撮影を頼み、画像はRelayの
  `GET /preview/<id>?token=`から取得する。自動では届かず、押したときだけ通信する
...
3. 端末が保存済みのRelayアドレスとトークンから絶対URLを組み立て、`GET /preview/<id>?token=`で画像を取る
